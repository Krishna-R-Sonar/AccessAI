...
const useStore = create((set) => ({
  ...
  userProgress: { points: 0, completedLessons: {}, badges: [] },
  ...
  factCheck: false,
  setFactCheck: (check) => set({ factCheck: check }),
  showReasoning: false,
  setShowReasoning: (show) => set({ showReasoning: show }),
  detailedMode: false,
  setDetailedMode: (mode) => set({ detailedMode: mode }),
  codeMode: false,
  setCodeMode: (mode) => set({ codeMode: mode }),
  auditMode: false,
  setAuditMode: (mode) => set({ auditMode: mode }),
  emotionDetection: false,
  setEmotionDetection: (detect) => set({ emotionDetection: detect }),
  criticalThinkingMode: false,
  setCriticalThinkingMode: (mode) => set({ criticalThinkingMode: mode }),
  offlineMode: false,
  setOfflineMode: (mode) => set({ offlineMode: mode }),
  simulationMode: false,
  setSimulationMode: (mode) => set({ simulationMode: mode }),
  aiLiteracyMode: false,
  setAiLiteracyMode: (mode) => set({ aiLiteracyMode: mode }),
  ...
  comments: {},
  setComments: (cmts) => set({ comments: cmts }),
  fullCodeView: null,
  setFullCodeView: (view) => set({ fullCodeView: view }),
  sidebarOpen: false,
...
function App() {
  const {
    ...
    emotionDetection,
    setEmotionDetection,
    criticalThinkingMode,
    setCriticalThinkingMode,
    offlineMode,
    setOfflineMode,
    simulationMode,
    setSimulationMode,
  ...
  }, [setMessages, setProjects, setUserProgress]);

  // Persist state to localStorage and backend
  useEffect(() => {
    localStorage.setItem('chatMessages', JSON.stringify(messages));
    localStorage.setItem('chatProjects', JSON.stringify(projects));
    localStorage.setItem('userProgress', JSON.stringify(userProgress));
    // Simulate backend save for userProgress
    saveProgressToBackend(userProgress);
  ...
  // Simulate saving userProgress to backend
  ...
  // Helper to update messages and projects
  const updateMessagesAndProjects = useCallback(
    (newMessage) => {
      ...
      if (currentProject) {
        setProjects((prev) => {
          ...
          if (project) {
            ...
            updated.push({ name: currentProject, messages: [newMessage] });
          }
          return updated;
        });
      }
    },
    [currentProject, setMessages, setProjects]
  );
  ...
  const handleSubmit = useCallback(
    async (textToProcess, isStudyGuide = false) => {
      ...
      updateMessagesAndProjects(userMessage);
      ...
      try {
        if (offlineMode) {
          ...
          updateMessagesAndProjects(assistantMessage);
          setLoading(false);
          setFileName('');
          return;
        }
        ...
        const response = await fetch(`${API_URL}/chat`, {
          ...
          headers: {
            ...
            Authorization: `Bearer ${localStorage.getItem('token') || 'mock-token'}`, // Mock token for now
          },
          body: JSON.stringify({
            messages,
            input: prompt,
        ...
        const assistantMessage = {
          role: 'assistant',
          content: `${errorMessage} [Retry]`,
          timestamp: new Date().toLocaleTimeString(),
          retry: () => handleSubmit(textToProcess, isStudyGuide),
        };
        updateMessagesAndProjects(assistantMessage);
      } finally {
        setLoading(false);
  ...
  const handleFileUpload = useCallback(
    async (event) => {
      ...
      try {
        ...
        if (file.type === 'text/plain') {
          ...
          updateMessagesAndProjects({
            role: 'assistant',
            content: 'Unsupported file type. Please upload a .txt, .pdf, or image file.',
            timestamp: new Date().toLocaleTimeString(),
          });
          setFileName('');
          setLoading(false);
        ...
        updateMessagesAndProjects({
          role: 'assistant',
          content: 'Failed to process the file. Please try a different file.',
          timestamp: new Date().toLocaleTimeString(),
        });
        setFileName('');
      } finally {
    ...
    [handleSubmit, setFileName, setLoading, updateMessagesAndProjects]
  );
  ...
  const generateStudyGuide = () => {
    ...
    if (lastMessage && lastMessage.role === 'user') {
      ...
      updateMessagesAndProjects({
        role: 'assistant',
        content: 'Please provide some notes to generate a study guide.',
        timestamp: new Date().toLocaleTimeString(),
      });
    }
  };
...
Would you like to learn more about a specific AI topic?
  ...
  }, [selectedLanguage, userProgress.completedLessons]);

  // Memoized message rendering for performance
  const renderMessage = useCallback(
    ({ index, style }) => {
      const msg = currentProject ? projects.find((p) => p.name === currentProject)?.messages[index] || messages[index] : messages[index];
      return (
        <div
          style={style}
            ...
            {msg.role === 'assistant' ? (
              <>
                ...
                <div className="response-actions">
                  ...
                  {msg.retry && (
                    <button onClick={msg.retry} className="action-button" aria-label="Retry request">
                      Retry
                    </button>
                  )}
                </div>
          ...
          <div className="chat-container">
            <div className="messages">
              <FixedSizeList
                ...
                width="100%"
                itemCount={
                  (currentProject ? projects.find((p) => p.name === currentProject)?.messages || messages : messages).length
                }
                itemSize={150}
              >
                {renderMessage}
